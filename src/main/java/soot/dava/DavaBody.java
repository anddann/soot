...
public class DavaBody extends Body {
  ...
  DavaBody(Body body, Options myOptions, Printer myPrinter, MonitorConverter myMonitorConverter,
    ...
    // prime the analysis
    AugmentedStmtGraph asg = new AugmentedStmtGraph(new BriefUnitGraph(this, myPhaseDumper), new TrapUnitGraph(this, myPhaseDumper));
    ...
    while (true) {
      try {
        this.myCycleFinder.find(this, asg, SET, primTypeCollector );
        this.myIfFinder.find(this, asg, SET, primTypeCollector );
        this.mySwitchFinder.find(this, asg, SET, primTypeCollector );
        this.mySynchronizedBlockFinder.find(this, asg, SET, primTypeCollector );
        this.myExceptionFinder.find(this, asg, SET, primTypeCollector );
        this.mySequenceFinder.find(this, asg, SET, primTypeCollector );
        this.myLabeledBlockFinder.find(this, asg, SET, primTypeCollector );
        this.myAbruptEdgeFinder.find(this, asg, SET, primTypeCollector );
  ...
  private void applyASTAnalyses(ASTNode AST) {
    ...
    if (force) {
      ...
      new FinalFieldDefinition((ASTMethodNode) AST,  myTryContentsFinder, myASTWalker, myScene, constantFactory);
      ...
      for (Unit u : getUnits()) {
        ...
        if (s instanceof IfStmt) {
          ...
          if (ds.getRightOp() instanceof IntConstant) {
            ds.getRightOpBox().setValue(constantFactory.createDIntConstant(((IntConstant) ds.getRightOp()).value, ds.getLeftOp().getType()));
          }
        }
    ...
    if (rightOp instanceof IntConstant) {
      if ((leftOp instanceof IntConstant) == false) {
        ...
        if (boe instanceof ConditionExpr) {
          rightOpBox.setValue(constantFactory.createDIntConstant(((IntConstant) rightOp).value, leftOp.getType()));
        } else {
      ...
      if (boe instanceof ConditionExpr) {
        leftOpBox.setValue(constantFactory.createDIntConstant(((IntConstant) leftOp).value, rightOp.getType()));
      } else {
  ...
  private void javafy_invoke_expr(ValueBox vb) {
    ...
    if (ie instanceof InstanceInvokeExpr) {
      ...
      if (ie instanceof VirtualInvokeExpr) {
        ...
        vb.setValue(new DInterfaceInvokeExpr(iie.getBase(), iie.getMethodRef(), iie.getArgs(),  myScene));
      } else {
      ...
      if (sie instanceof NewInvokeExpr) {
        ...
        addToImportList(className);
        vb.setValue(new DNewInvokeExpr((RefType) nie.getType(), nie.getMethodRef(), nie.getArgs(),  myScene));
      } else {
