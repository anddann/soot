    ...
    if (aggCond instanceof ASTAndCondition) {
      ...
      if (leftBool != null && rightBool != null) {
        ...
        if (leftBool.booleanValue() && rightBool.booleanValue()) {
          // both are true
          return new ASTUnaryCondition(constantFactory.createDIntConstant(1, primTypeCollector.getBooleanType()), myGrimp, primTypeCollector);
        } else {
          // atleast one of the two is false
          return new ASTUnaryCondition(constantFactory.createDIntConstant(0, primTypeCollector.getBooleanType()), myGrimp, primTypeCollector);
        }
      }
