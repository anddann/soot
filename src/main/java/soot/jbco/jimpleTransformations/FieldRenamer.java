...
public class FieldRenamer extends SceneTransformer implements IJbcoTransform {
    ...
    protected void setBooleanTo(SootClass sootClass, SootField field, boolean value) {
        ...
        if (field.getType() instanceof IntegerType) {
            ...
            units.addFirst(Jimple.newInvokeStmt(
                    Jimple.newSpecialInvokeExpr(bool, booleanWrapperConstructor.makeRef(), constantFactory.createIntConstant(value ? 1 : 0))));
     ...
     * already generated.
     *
     * @param originalName the original field name. If {@code null} then will be the same as generated one
