...
public class ReflectiveCallsInliner extends SceneTransformer {
  ...
  //FIXME male singlenton
  public ReflectiveCallsInliner(Scene myScene, PackageNamer myPackageNamer, Options myOptions, PhaseOptions myPhaseOptions, CopyPropagator myCopyPropagator, DeadAssignmentEliminator myDeadAssignmentEliminator, UnusedLocalEliminator myUnusedLocalEliminator, NopEliminator myNopEliminator, PrimTypeCollector primTypeCollector, ConstantFactory constantFactory, Printer myPrinter) {
    ...
    protected void internalTransform(String phaseName, @SuppressWarnings("rawtypes") Map options) {
        if (!initialized) {
            ...
            UNINTERPRETED_METHOD = myScene.makeMethodRef(myScene.getSootClass("soot.rtlib.tamiflex.OpaquePredicate"),
                    "getFalse", Collections.<Type>emptyList(), myScene.getPrimTypeCollector().getBooleanType(), true);
        ...
        for (SootMethod m : RTI.methodsContainingReflectiveCalls()) {
            {
                if (!RTI.classForNameClassNames(m).isEmpty()) {
                    SootFieldRef fieldRef = myScene.makeFieldRef(reflCallsClass, "classForName", RefType.v("java.util.Set",myScene), true);
                    Local setLocal = localGen.generateLocal(RefType.v("java.util.Set",myScene));
            ...
            }
            {
                if (!RTI.classNewInstanceClassNames(m).isEmpty()) {
                    SootFieldRef fieldRef
                            = myScene.makeFieldRef(reflCallsClass, "classNewInstance", RefType.v("java.util.Set",myScene), true);
                    Local setLocal = localGen.generateLocal(RefType.v("java.util.Set",myScene));
                    newUnits.add(Jimple.newAssignStmt(setLocal, Jimple.newStaticFieldRef(fieldRef)));
            ...
            }
            {
                if (!RTI.constructorNewInstanceSignatures(m).isEmpty()) {
                    SootFieldRef fieldRef
                            = myScene.makeFieldRef(reflCallsClass, "constructorNewInstance", RefType.v("java.util.Set",myScene), true);
                    Local setLocal = localGen.generateLocal(RefType.v("java.util.Set",myScene));
                    newUnits.add(Jimple.newAssignStmt(setLocal, Jimple.newStaticFieldRef(fieldRef)));
            ...
            }
            {
                if (!RTI.methodInvokeSignatures(m).isEmpty()) {
                    SootFieldRef fieldRef = myScene.makeFieldRef(reflCallsClass, "methodInvoke", RefType.v("java.util.Set",myScene), true);
                    Local setLocal = localGen.generateLocal(RefType.v("java.util.Set",myScene));
                    newUnits.add(Jimple.newAssignStmt(setLocal, Jimple.newStaticFieldRef(fieldRef)));
    ...
    private SootMethod createNewMethod(Kind callKind, String target, Type fieldSetGetType) {
        ...
        Body newBody = Jimple.newBody(newMethod,myPrinter, myOptions);
        ...
        switch (callKind) {
            ...
            case ClassNewInstance: {
                ...
                RefType targetType = RefType.v(target,myScene);
        ...
        switch (callKind) {
            ...
            case MethodInvoke: {
                ...
                if (method.isStatic()) {
                    ...
                    invokeExpr = Jimple.newVirtualInvokeExpr(freshLocal, method.makeRef(), Arrays.<Value>asList(paramLocals),myOptions);
                }
                if (method.getReturnType().equals(primTypeCollector.getVoidType())) {
                    ...
                    AssignStmt assignStmt = Jimple.newAssignStmt(retLocal, constantFactory.getNullConstant());
    ...
    private void insertCastOrUnboxingCode(Local lhs, Local rhs, Chain<Unit> newUnits) {
        ...
        if (lhs.getType() instanceof PrimType) {
            if ((rhs.getType() instanceof PrimType)) {
                ...
                newUnits.add(Jimple.newAssignStmt(lhs, Jimple.newVirtualInvokeExpr(rhs, ref,myOptions)));
            }
        }
    }
    ...
    private void unboxParameter(Local argsArrayLocal, int paramIndex, Local[] paramLocals, Type paramType,
        ...
        if (paramType instanceof PrimType) {
            ...
            newUnits.add(cast);
            VirtualInvokeExpr unboxInvokeExpr = Jimple.newVirtualInvokeExpr(castedLocal, ref,myOptions);
