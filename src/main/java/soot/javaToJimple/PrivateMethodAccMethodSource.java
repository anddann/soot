...
public class PrivateMethodAccMethodSource implements soot.MethodSource {
  ...
  public soot.Body getBody(soot.SootMethod sootMethod, String phaseName) {
    ...
    soot.SootMethodRef meth
        = soot.myScene.makeMethodRef(((soot.RefType) Util.getSootType(methodInst.container())).getSootClass(),
    ...
    if (methodInst.flags().isStatic()) {
      invoke = soot.jimple.myJimple.newStaticInvokeExpr(meth, methParams);
    } else {
      invoke = soot.jimple.myJimple.newSpecialInvokeExpr(base, meth, methParams);
    }

    soot.jimple.Stmt stmt = null;
    if (!(type instanceof soot.VoidType)) {
      stmt = soot.jimple.myJimple.newAssignStmt(returnLocal, invoke);
    } else {
      stmt = soot.jimple.myJimple.newInvokeStmt(invoke);
    }
    body.getUnits().add(stmt);
    ...
    if (!(type instanceof soot.VoidType)) {
      retStmt = soot.jimple.myJimple.newReturnStmt(returnLocal);
    } else {
      retStmt = soot.jimple.myJimple.newReturnVoidStmt();
    }
