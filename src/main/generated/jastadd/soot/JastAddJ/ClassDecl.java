...
/**
 * @production ClassDecl : {@link ReferenceType} ::= <span class="component">{@link Modifiers}</span>
...
public class ClassDecl extends ReferenceType implements Cloneable {
          ...
          for (Iterator i2 = interfacesMethodsSignature(m.signature()).iterator(); i2.hasNext();) {
            ...
            if (!n.mayOverrideReturn(m) && !m.mayOverrideReturn(n))
              error("Xthe return type of method " + m.signature() + " in " + m.hostType().typeName()
                  + " does not match the return type of method " + n.signature() + " in " + n.hostType().typeName()
  ...
  private void interfaceMethodCompatibleWithInherited(MethodDecl m, MethodDecl n) {
    ...
    if (!n.isAbstract() && !n.isPublic())
      error("Xoverriding access modifier error for " + m.signature() + " in " + m.hostType().typeName() + " and "
          + n.hostType().typeName());
    ...
    if (!n.isAbstract()) {
      ...
      for (int i = 0; i < n.getNumException(); i++) {
        ...
        if (!found && e.type().isUncheckedException())
          error("X" + n.signature() + " in " + n.hostType().typeName()
              + " may not throw more checked exceptions than overridden method " + m.signature() + " in "
              + m.hostType().typeName());
      }
    }
  }
  ...
  public TypeDecl makeGeneric(Signatures.ClassSignature s) {
    if (s.hasFormalTypeParameters()) {
      ...
      node.setChild(new GenericClassDecl(getModifiersNoTransform(), getID(),
          ...
          getBodyDeclListNoTransform(), s.typeParameters(), myScene, myJimple, mySootResolver, myPackageNamer, myOptions,
          primTypeCollector, constantFactory, myPhaseOptions), index);
  ...
  public ClassDecl substitutedClassDecl(Parameterization parTypeDecl) {
    ClassDecl c = new ClassDeclSubstituted((Modifiers) getModifiers().fullCopy(), getID(),
        ...
        // ES: new List(),
        this, myScene, myJimple, mySootResolver, myPackageNamer, myOptions, primTypeCollector, constantFactory,
        myPhaseOptions
   ...
   */
  public ClassDecl(Scene myScene, Jimple myJimple, SootResolver mySootResolver, PackageNamer myPackageNamer,
   ...
   * 
   */
  public ClassDecl(Modifiers p0, String p1, Opt<Access> p2, List<Access> p3, List<BodyDecl> p4, Scene myScene,
      Jimple myJimple, SootResolver mySootResolver, PackageNamer myPackageNamer, Options myOptions,
   ...
   * 
   */
  public ClassDecl(Modifiers p0, Symbol p1, Opt<Access> p2, List<Access> p3, List<BodyDecl> p4, Scene myScene,
      Jimple myJimple, SootResolver mySootResolver, PackageNamer myPackageNamer, Options myOptions,
      PrimTypeCollector primTypeCollector, ConstantFactory constantFactory, PhaseOptions myPhaseOptions) {
    super(myScene, myJimple, mySootResolver, myPackageNamer, myOptions, primTypeCollector, constantFactory, myPhaseOptions);
    this.myScene = myScene;
    ...
    for (Iterator outerIter = interfacesIterator(); outerIter.hasNext();) {
      ...
      for (Iterator iter = typeDecl.methodsIterator(); iter.hasNext();) {
        ...
        if (!m.isPrivate() && m.accessibleFrom(this) && !localMethodsSignatureMap().containsKey(m.signature())) {
          if (!(m instanceof MethodDeclSubstituted)
              || !localMethodsSignatureMap().containsKey(m.sourceMethodDecl().signature())) {
            if (allMethodsAbstract((SimpleSet) map.get(m.signature())) && (!(m instanceof MethodDeclSubstituted)
  ...
  private boolean castingConversionTo_compute(TypeDecl type) {
    ...
    return false;
    /*
     * else if(unboxingConversionTo(type)) return true; return false;
     */
  }
