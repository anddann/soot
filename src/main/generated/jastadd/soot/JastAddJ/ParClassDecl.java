...
/**
 * @production ParClassDecl : {@link ClassDecl} ::= <span class="component">Argument:{@link Access}*</span>
 *             <span class="component">[SuperClassAccess:{@link Access}]</span>
 *             <span class="component">Implements:{@link Access}*</span> <span class="component">{@link BodyDecl}*</span>;
 * @ast node
...
public class ParClassDecl extends ClassDecl implements Cloneable, ParTypeDecl, MemberSubstitutor {
   ...
   * 
   */
  public ParClassDecl(Scene myScene, Jimple jimple, SootResolver sootResolver, PrimTypeCollector primTypeCollector,
      Options myOptions, PackageNamer myPackageNamer, ConstantFactory constantFactory, PhaseOptions phaseOptions) {
    super(myScene, jimple, sootResolver, myPackageNamer, myOptions, primTypeCollector, constantFactory, phaseOptions);
   ...
   * 
   */
  public ParClassDecl(Modifiers p0, String p1, List<Access> p2, Scene myScene, Jimple jimple, SootResolver sootResolver,
      PrimTypeCollector primTypeCollector, Options myOptions, PackageNamer myPackageNamer, ConstantFactory constantFactory,
   ...
   * 
   */
  public ParClassDecl(Modifiers p0, beaver.Symbol p1, List<Access> p2, Scene myScene, Jimple jimple,
      SootResolver sootResolver, PrimTypeCollector primTypeCollector, Options myOptions, PackageNamer myPackageNamer,
      ConstantFactory constantFactory, PhaseOptions phaseOptions) {
    super(myScene, jimple, sootResolver, myPackageNamer, myOptions, primTypeCollector, constantFactory, phaseOptions);
    setChild(p0, 0);
    setID(p1);
    setChild(p2, 1);
  ...
  /**
   * Replaces the optional node for the SuperClassAccess child. This is the {@code Opt} node containing the child
   * SuperClassAccess, not the actual child!
   * 
   * @param opt
  ...
  /**
   * Retrieves the optional node for child SuperClassAccess. This is the {@code Opt} node containing the child
   * SuperClassAccess, not the actual child!
   * <p>
   * <em>This method does not invoke AST transformations.</em>
   * </p>
  ...
  private HashMap localMethodsSignatureMap_compute() {
    ...
    for (Iterator iter = original().localMethodsIterator(); iter.hasNext();) {
      ...
      /*
       * ES removing this: if(!decl.isStatic() && (decl.usesTypeVariable() || isRawType())) { BodyDecl b =
       * decl.substitutedBodyDecl(this); addBodyDecl(b); // Here we should access b through an ordinary // child accessor
       * instead of setting is$Final directly, // however doing so appears to cause unexpected behaviour! b.is$Final = true;
       * decl = (MethodDecl) b; } map.put(decl.signature(), decl); and replacing with:
       */
  ...
  private SimpleSet localFields_compute(String name) {
    ...
    for (Iterator iter = original().localFields(name).iterator(); iter.hasNext();) {
      ...
      /*
       * ES removing this: if(!f.isStatic() && (f.usesTypeVariable() || isRawType())) { BodyDecl b =
       * f.substitutedBodyDecl(this); addBodyDecl(b); // Here we should access b through an ordinary // child accessor
       * instead of setting is$Final directly, // however doing so appears to cause unexpected behaviour! b.is$Final = true;
       * f = (FieldDeclaration) b; } set = set.add(f); and replacing with:
       */
  ...
  private SimpleSet localTypeDecls_compute(String name) {
    ...
    for (Iterator iter = original().localTypeDecls(name).iterator(); iter.hasNext();) {
      ...
      /*
       * ES: removing this: if(t.isStatic()) set = set.add(t); else { BodyDecl b; TypeDecl typeDecl; if(t instanceof
       * ClassDecl) { ClassDecl classDecl = (ClassDecl)t; typeDecl = classDecl.substitutedClassDecl(this); b = new
       * MemberClassDecl((ClassDecl)typeDecl); addBodyDecl(b); // Here we should access b through an ordinary // child
       * accessor instead of setting is$Final directly, // however doing so appears to cause unexpected behaviour! b.is$Final
       * = true; set = set.add(typeDecl); } else if(t instanceof InterfaceDecl) { InterfaceDecl interfaceDecl =
       * (InterfaceDecl)t; typeDecl = interfaceDecl.substitutedInterfaceDecl(this); b = new
       * MemberInterfaceDecl((InterfaceDecl)typeDecl); addBodyDecl(b); // Here we should access b through an ordinary //
       * child accessor instead of setting is$Final directly, // however doing so appears to cause unexpected behaviour!
       * b.is$Final = true; set = set.add(typeDecl); } } and replacing with:
       */
      if (t.isStatic()) {
        set = set.add(t);
  ...
  private Collection constructors_compute() {
    ...
    for (Iterator iter = original().constructors().iterator(); iter.hasNext();) {
      ...
      /*
       * ES: removing this: BodyDecl b = c.substitutedBodyDecl(this); addBodyDecl(b); // Here we should access b through an
       * ordinary // child accessor instead of setting is$Final directly, // however doing so appears to cause unexpected
       * behaviour! b.is$Final = true; and replacing with:
       */
