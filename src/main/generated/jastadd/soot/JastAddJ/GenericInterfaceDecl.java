...
/**
 * @production GenericInterfaceDecl : {@link InterfaceDecl} ::= <span class="component">{@link Modifiers}</span>
 ...
 *             <span class="component">TypeParameter:{@link TypeVariable}*</span>;
 * @ast node
...
public class GenericInterfaceDecl extends InterfaceDecl implements Cloneable, GenericTypeDecl {
  ...
  public InterfaceDecl substitutedInterfaceDecl(Parameterization parTypeDecl) {
    GenericInterfaceDecl c = new GenericInterfaceDeclSubstituted((Modifiers) getModifiers().fullCopy(), getID(),
        ...
        new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
            myPhaseOptions), // delegates TypeParameter lookup to original
        this,
  ...
  public void toString(StringBuffer s) {
    ...
    /*
     * 
     * s.append(" instantiated with: "); for(int i = 0; i < getNumParTypeDecl(); i++) { if(i != 0) s.append(", ");
     * ParTypeDecl decl = getParTypeDecl(i); s.append("<"); for(int j = 0; j < decl.getNumArgument(); j++) { if(j != 0)
     * s.append(", "); s.append(decl.getArgument(j).type().fullName()); } s.append(">"); }
     */

    ppBodyDecls(s);

    /*
     * for(int i = 0; i < getNumParTypeDecl(); i++) { ParInterfaceDecl decl = getParTypeDecl(i); decl.toString(s); }
     */
  }
   ...
   * @param phaseOptions
   */
  public GenericInterfaceDecl(Scene scene, Jimple jimple, SootResolver sootresolver, PackageNamer packagenNamer,
      Options options, PrimTypeCollector primTypeCollector, ConstantFactory constantFactory, PhaseOptions phaseOptions) {
  ...
  public GenericInterfaceDecl(Modifiers p0, String p1, List<Access> p2, List<BodyDecl> p3, List<TypeVariable> p4,
      Scene scene, Jimple jimple, SootResolver sootresolver, PackageNamer packagenNamer, Options options,
  ...
  public TypeDecl lookupParTypeDecl(ArrayList list) {
    ...
    if (lookupParTypeDecl_ArrayList_list == null) {
      lookupParTypeDecl_ArrayList_list = new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector,
  ...
  private TypeDecl lookupParTypeDecl_compute(ArrayList list) {
     ...
     * ParInterfaceDecl typeDecl = (ParInterfaceDecl)createParTypeDeclStub(list); if (size <
     * createParTypeDeclStub_ArrayList_list.numChildren) { createParTypeDeclBody(list, typeDecl); } return typeDecl;
     */

    ParInterfaceDecl typeDecl = list.size() == 0 ? new RawInterfaceDecl(myScene,myJimple,mySootResolver,myPackageNamer,myOptions,primTypeCollector,constantFactory,myPhaseOptions) : new ParInterfaceDecl(myScene,myJimple,mySootResolver,myPackageNamer,myOptions,primTypeCollector,constantFactory,myPhaseOptions);
