...
/**
 * @production TypeDecl : {@link ASTNode} ::= <span class="component">{@link Modifiers}</span>
 *             <span class="component">&lt;ID:String&gt;</span> <span class="component">{@link BodyDecl}*</span>;
 * @ast node
...
public abstract class TypeDecl extends ASTNode<ASTNode> implements Cloneable, SimpleSet, Iterator, VariableScope {
  ...
  public MethodDecl addMemberMethod(MethodDecl m) {
    ...
    /*
     * HashMap map = methodsNameMap(); ArrayList list = (ArrayList)map.get(m.name()); if(list == null) { list = new
     * ArrayList(4); map.put(m.name(), list); } list.add(m); if(!memberMethods(m.name()).contains(m)) throw new
     * Error("The method " + m.signature() + " added to " + typeName() + " can not be found using lookupMemberMethod");
     */
  }
  ...
  public Access createQualifiedAccess() {
    if (isLocalClass() || isAnonymous()) {
      return new TypeAccess(name(), myScene, primTypeCollector, myJimple);
    } else if (!isTopLevelType()) {
      return enclosingType().createQualifiedAccess()
          .qualifiesAccess(new TypeAccess(name(), myScene, primTypeCollector, myJimple));
    } else {
      return new TypeAccess(packageName(), name(), myScene, primTypeCollector, myJimple);
    }
  }
    ...
    for (Iterator iter1 = localMethodsIterator(); iter1.hasNext();) {
      ...
      for (Iterator i2 = ancestorMethods(m.signature()).iterator(); i2.hasNext();) {
        ...
        if (m.overrides(decl)) {
          ...
          if (!m.mayOverrideReturn(decl))
            target.error("the return type of method " + m.signature() + " in " + m.hostType().typeName()
                + " does not match the return type of method " + decl.signature() + " in " + decl.hostType().typeName()
          ...
          for (int i = 0; i < m.getNumException(); i++) {
            ...
            if (!found && e.type().isUncheckedException())
              target.error(m.signature() + " in " + m.hostType().typeName()
                  + " may not throw more checked exceptions than overridden method " + decl.signature() + " in "
          ...
          // regardless of overriding
          if (decl.isFinal())
            target.error("method " + m.signature() + " in " + hostType().typeName() + " can not override final method "
        ...
        if (m.hides(decl)) {
          ...
          if (decl.isPublic() && !m.isPublic())
            target.error(
                "hiding access modifier error: public method " + decl.signature() + " in " + decl.hostType().typeName()
                    + " is hidden by non public method " + m.signature() + " in " + m.hostType().typeName());
          // 8.4.6.3
          if (decl.isProtected() && !(m.isPublic() || m.isProtected()))
            target.error(
                "hiding access modifier error: protected method " + decl.signature() + " in " + decl.hostType().typeName()
                    + " is hidden by non (public|protected) method " + m.signature() + " in " + m.hostType().typeName());
          // 8.4.6.3
          if ((!decl.isPrivate() && !decl.isProtected() && !decl.isPublic()) && m.isPrivate())
            target.error(
                "hiding access modifier error: default method " + decl.signature() + " in " + decl.hostType().typeName()
                    + " is hidden by private method " + m.signature() + " in " + m.hostType().typeName());
          if (decl.isFinal())
            target.error("method " + m.signature() + " in " + hostType().typeName() + " can not hide final method "
                + decl.signature() + " in " + decl.hostType().typeName());
        }
      }
    }
  ...
  public Access substitute(Parameterization parTypeDecl) {
    ...
    return enclosingType().substitute(parTypeDecl)
        .qualifiesAccess(new TypeAccess(name(), myScene, primTypeCollector, myJimple));
  }
  ...
  public void addEnclosingVariables() {
    ...
    for (Iterator iter = enclosingVariables().iterator(); iter.hasNext();) {
      ...
      Modifiers m = new Modifiers(myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory,
          primTypeCollector, mySootResolver);
    ...
    createAssertionsDisabled = new FieldDeclaration(
        new Modifiers(
            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                myPhaseOptions).add(new Modifier("public")).add(
                    new Modifier("static")).add(
                        new Modifier("final")),
            myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory, primTypeCollector,
            mySootResolver),
        new PrimitiveTypeAccess("boolean"), "$assertionsDisabled",
        new Opt(new LogNotExpr(topLevelType().createQualifiedAccess()
            .qualifiesAccess(new ClassAccess().qualifiesAccess(new MethodAccess("desiredAssertionStatus", list, myScene,
                primTypeCollector, myJimple, myOptions, myPackageNamer, constantFactory, mySootResolver, myPhaseOptions))))),
  ...
  public FieldDeclaration createStaticClassField(String name) {
    ...
    FieldDeclaration f = new FieldDeclaration(
        new Modifiers(
            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                myPhaseOptions).add(new Modifier("public")).add(new Modifier("static")),
            myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory, primTypeCollector,
            mySootResolver),
        lookupType("java.lang", "Class").createQualifiedAccess(), name, new Opt(), myScene, myOptions, myPackageNamer,
        myJimple, primTypeCollector, constantFactory, mySootResolver, myPhaseOptions) {
            ...
            myPhaseOptions)
                .add(new ParameterDeclaration(
                    new Modifiers(new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory,
                        mySootResolver, myPhaseOptions), myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple,
                        constantFactory, primTypeCollector, mySootResolver),
                        ...
                        new Block(
                            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory,
                                mySootResolver, myPhaseOptions)
                                    .add(new ReturnStmt(new Opt(lookupType("java.lang", "Class").createQualifiedAccess()
                                        .qualifiesAccess(new MethodAccess(
                                            "forName",
                                            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector,
                                                constantFactory, mySootResolver, myPhaseOptions).add(new VarAccess("name")),
                                            myScene, primTypeCollector, myJimple, myOptions, myPackageNamer, constantFactory,
                                            mySootResolver, myPhaseOptions))))),
                            ...
                            myPhaseOptions).add(
                                new BasicCatch(
                                    new ParameterDeclaration(new Modifiers(
                                        ...
                                        myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory,
                                        primTypeCollector, mySootResolver),
                                        lookupType("java.lang", "ClassNotFoundException").createQualifiedAccess(), "e"),
                                    new Block(
                                        new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector,
                                            constantFactory, mySootResolver, myPhaseOptions)
                                                .add(new ThrowStmt(new ClassInstanceExpr(
                                                    lookupType("java.lang", "NoClassDefFoundError").createQualifiedAccess(),
                                                    new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector,
                                                        constantFactory, mySootResolver, myPhaseOptions).add(
                                                            new VarAccess("e").qualifiesAccess(new MethodAccess("getMessage",
                                                                new List(myScene, myOptions, myPackageNamer, myJimple,
                                                                    primTypeCollector, constantFactory, mySootResolver,
                                                                    myPhaseOptions),
                                                                myScene, primTypeCollector, myJimple, myOptions,
                                                                myPackageNamer, constantFactory, mySootResolver,
                                                                myPhaseOptions))),
                                                    new Opt(), myScene, constantFactory, primTypeCollector, myJimple,
                                                    myPackageNamer, myOptions, mySootResolver, myPhaseOptions))),
  ...
  public void jimplify1phase2() {
    if (needsClinit() && !getSootClassDecl().declaresMethod("<clinit>", new ArrayList())) {
      clinit = myScene.makeSootMethod("<clinit>", new ArrayList(), myScene.getPrimTypeCollector().getVoidType(),
          soot.Modifier.STATIC, new ArrayList());
  ...
  public void jimplify2clinit() {
    ...
    Body b = new Body(this, body, this, constantFactory);
    for (int i = 0; i < getNumBodyDecl(); i++) {
   ...
   */
  public TypeDecl(Scene myScene, Jimple myJimple, SootResolver mySootResolver, Options myOptions,
  ...
  public void init$Children() {
    ...
    setChild(new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
        myPhaseOptions), 1);
  }
  ...
  public TypeDecl(Modifiers p0, String p1, List<BodyDecl> p2, Scene myScene, Jimple myJimple, SootResolver mySootResolver,
      Options myOptions, PrimTypeCollector primTypeCollector, PackageNamer myPackageNamer, ConstantFactory constantFactory,
      PhaseOptions myPhaseOptions) {
    this.myScene = myScene;
    this.myJimple = myJimple;
    this.mySootResolver = mySootResolver;
        ...
        for (Iterator i2 = list.listIterator(i); i2.hasNext();) {
          ...
          if (decl2 instanceof ParInterfaceDecl) {
            ...
            if (p != q && p.genericDecl() == q.genericDecl() && !p.sameArgument(q))
              error(p.genericDecl().name() + " cannot be inherited with different arguments: " + p.typeName() + " and "
                  + q.typeName());
          }
        }
      }
  ...
  private boolean refined_TypeConversion_TypeDecl_assignConversionTo_TypeDecl_Expr(TypeDecl type, Expr expr) {
    ...
    if (sourceIsConstant && (isInt() || isChar() || isShort() || isByte())
        && (type.isByte() || type.isShort() || type.isChar()) && narrowingConversionTo(type) && expr.representableIn(type))
      return true;
  ...
  private TypeDecl arrayType_compute() {
    ...
    if (clone != null) {
      body.add(
          ...
          new MethodDeclSubstituted(
              new Modifiers(
                  new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                      myPhaseOptions).add(new Modifier("public")),
                  myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory, primTypeCollector,
                  mySootResolver),
              new ArrayTypeAccess(createQualifiedAccess()), "clone",
              new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                  myPhaseOptions),
              new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                  myPhaseOptions),
    ...
    Modifiers aPublic = new Modifiers(
        new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
            myPhaseOptions).add(new Modifier("public")),
        myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory, primTypeCollector, mySootResolver);
    List list = new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
        myPhaseOptions);
    TypeDecl typeDecl = new ArrayDecl(aPublic, name, new Opt(typeObject().createQualifiedAccess()), // [SuperClassAccess]
        list.add(typeCloneable().createQualifiedAccess()).add(typeSerializable().createQualifiedAccess()), // Implements*
        body // BodyDecl*
        , myScene, myJimple, mySootResolver, myOptions, myPackageNamer, primTypeCollector, constantFactory, myPhaseOptions);
    return typeDecl;
  }
  ...
  /**
   * Unfortunately the concept of checked vs. unchecked exceptions has been inverted in JastAddJ compared to the Java
   * specification. This is a slightly unfortunate design flaw which we cannot change at this time.
   * 
   * @attribute syn
   * @aspect ExceptionHandling
  ...
  public boolean assignConversionTo(TypeDecl type, Expr expr) {
    ...
    try {
      ...
      boolean sourceIsConstant = expr != null ? expr.isConstant() : false;
      if (sourceIsConstant && (isInt() || isChar() || isShort() || isByte()) && (t.isByte() || t.isShort() || t.isChar())
  ...
  private boolean castingConversionTo_compute(TypeDecl type) {
    ...
    return false;
    /*
     * else if(boxingConversionTo(type)) return true; else if(unboxingConversionTo(type)) return true; return false;
     */
  }
  ...
  /*
   * It is a compile-time error if the return type of a method declared in an annotation type is any type other than one of
   * the following: one of the primitive types, String, Class and any invocation of Class, an enum type (\ufffd8.9), an
   * annotation type, or an array (\ufffd10) of one of the preceding types.* @attribute syn
   * 
   * @aspect Annotations
   * 
  ...
  /*
   * An element type T is commensurate with an element value V if and only if one of the following conditions is true: T is
   * an array type E[] and either: o V is an ElementValueArrayInitializer and each ElementValueInitializer (analogous to a
   * variable initializer in an array initializer) in V is commensurate with E. Or o V is an ElementValue that is
   * commensurate with T. The type of V is assignment compatible (\ufffd5.2) with T and, furthermore: o If T is a primitive
   * type or String, V is a constant expression (\ufffd15.28). o V is not null. o if T is Class, or an invocation of Class,
   * and V is a class literal (\ufffd15.8.2). o If T is an enum type, and V is an enum constant. * @attribute syn
   * 
   * @aspect Annotations
   * 
  ...
  /*
   * NumericTypes, BooleanTypes TypeChecking (ensure that an expression of a certain type is valid in a particular context)
   * TypeComputation (compute the type of an expression) CodeGeneration (output code including implicit type conversions and
   * promotions)
   * 
   * NumericTypes: binaryNumericPromotion, unaryNumericPromotion, assignmentConversion, methodInvocationConversion,
   * castingConversion numeric operations that do not use these kinds of conversions and promotions explicitly need to be
   * refined BooleanTypes: assignmentConversion, methodInvocationConversion, castingConversion
   * 
   * @attribute syn
   * 
   * @aspect AutoBoxing
  ...
  /*
   * 1) It is a compile-time error to attempt to explicitly instantiate an enum type (\ufffd\ufffd\ufffd15.9.1).
   * 
   * @attribute syn
   * 
   * @aspect Enums
  ...
  private SootClass getSootClassDecl_compute() {
    ...
    if (compilationUnit().fromSource()) {
      ...
      /*
       * 
       * RefType type = (RefType) myScene.getRefType(jvmName()); SootClass toReturn = null; if( type != null ) toReturn =
       * type.getSootClass(); if(toReturn != null) { return toReturn; } SootClass c = new SootClass(jvmName());
       * c.setPhantom(true); myScene.addClass(c); return c;
       */

      // return myScene.getSootClass(jvmName());
      /*
       * SootClass sc = myScene.loadClass(jvmName(), SootClass.SIGNATURES); sc.setLibraryClass(); return sc;
       */
    }
  ...
  private MethodDecl createEnumMethod_compute(TypeDecl enumDecl) {
    MethodDecl m = new MethodDecl(
        new Modifiers(
            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                myPhaseOptions).add(new Modifier("static")).add(new Modifier("final")).add(new Modifier("private")),
            myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory, primTypeCollector,
            mySootResolver),
        ...
        new Opt(new Block(
            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                myPhaseOptions)
                    .add(
                        new IfStmt(
                            new EQExpr(createEnumArray(enumDecl).createBoundFieldAccess(), new NullLiteral("null")),
                            AssignExpr.asStmt(
                                createEnumArray(enumDecl).createBoundFieldAccess(),
                                new ArrayCreationExpr(new ArrayTypeWithSizeAccess(typeInt().createQualifiedAccess(),
                                    enumDecl.createQualifiedAccess()
                                        .qualifiesAccess(new MethodAccess("values",
                                            new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector,
                                                constantFactory, mySootResolver, myPhaseOptions),
                                            myScene, primTypeCollector, myJimple, myOptions, myPackageNamer, constantFactory,
                                            mySootResolver, myPhaseOptions))
    ...
    list.insertChild(
        new TryStmt(
            new Block(
                new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                    myPhaseOptions)
                        .add(AssignExpr.asStmt(
                            createEnumArray(e.hostType()).createBoundFieldAccess().qualifiesAccess(new ArrayAccess(e
                                .createBoundFieldAccess()
                                .qualifiesAccess(new MethodAccess("ordinal",
                                    new List(myScene, myOptions, myPackageNamer, myJimple, primTypeCollector,
                                        constantFactory, mySootResolver, myPhaseOptions),
                                    myScene, primTypeCollector, myJimple, myOptions, myPackageNamer, constantFactory,
                                    mySootResolver, myPhaseOptions)))),
                ...
                myPhaseOptions)
                    .add(new BasicCatch(
                        ...
                        new Block(
                            ...
                            myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
                            myPhaseOptions))),
            new Opt(), myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver,
            myPhaseOptions),
        list.getNumChild() - 1);
    return i.intValue();
  }
  ...
  private FieldDeclaration createEnumArray_compute(TypeDecl enumDecl) {
    FieldDeclaration f = new FieldDeclaration(
        new Modifiers(
            ...
            myPhaseOptions, myScene, myOptions, myPackageNamer, myJimple, constantFactory, primTypeCollector,
            mySootResolver),
        typeInt().arrayType().createQualifiedAccess(), "$SwitchMap$" + enumDecl.fullName().replace('.', '$'), new Opt(),
        myScene, myOptions, myPackageNamer, myJimple, primTypeCollector, constantFactory, mySootResolver, myPhaseOptions);
  ...
  /**
   * A type is reifiable if it either refers to a non-parameterized type, is a raw type, is a parameterized type with only
   * unbound wildcard parameters or is an array type with a reifiable type parameter.
   *
   * @see "JLSv3 &sect;4.7"
   * @attribute syn
   * @aspect SafeVarargs
  ...
  /**
   * An unchecked conversion occurs when converting from a raw type G to a generic type G<T1, ..., Tn>.
   * 
   * @attribute syn
